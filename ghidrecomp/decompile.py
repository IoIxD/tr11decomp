...
def decompile_func(func: 'ghidra.program.model.listing.Function',
    ...
    result: "DecompileResults" = decompilers[thread_id].decompileFunction(
...
def decompile_to_single_file(path: Path,
    ...
    decompiler = CppExporter(create_header, create_file,
                             emit_types, exclude_tags, tags)

    decompiler.export(c_file, prog, prog.getMemory(), monitor)


# the function for actually writing functions to files.
def file_write(output_path, name, decomp, sig):
    # skip those functions that are just "empty infinite loops"
    if "Do nothing block with infinite loop" in decomp:
        return
    # those functions that start with "__" and "_." are internal functions. don't include
    # them.
    if name.startswith("__") or name.startswith("_."):
        return
    # skip libpng functions
    if "png" in name.lower():
        return
    # skip opengl/more libpng functions
    if (name.startswith("gl") or name.startswith("png")):
        return
    ...
    (output_path / (namespace + '.c')).open("a").write(decomp+"\n")
...
def decompile(args: Namespace):
    ...
    with open_program(bin_path, project_location=project_location, project_name=bin_path.name, analyze=False) as flat_api:
        ...
        if skip_count > 0:
            print(
                f'Skipped {skip_count} functions that failed to match any of {args.filters}')

        if args.cppexport:
            print(
                f"Decompiling {len(all_funcs)} functions using Ghidra's CppExporter")
            c_file = Path(args.output_path) / Path(bin_path.name + '.c')
            start = time()
            decompile_to_single_file(c_file, program)
            print(
                f'Decompiled {len(all_funcs)} functions for {program.name} in {time() - start}')
            print(f"Wrote results to {c_file} and {c_file.stem + '.h'}")
        else:
            print(
                f'Decompiling {len(all_funcs)} functions using {thread_count} threads')
            ...
            start = time()
            with concurrent.futures.ThreadPoolExecutor(max_workers=thread_count) as executor:
                futures = (executor.submit(file_write, output_path, name, decomp, sig)
